...
const Subscribe = () => {
  ...
  console.log(params);
  const [savedNotes] = useState(localStorage.getItem(`list${params.id1}${params.model1}`))
 ...
 const deleteBid=(id)=> {
  ...
  useEffect(() => {
    localStorage.setItem(`list${params.id1}${params.model1}`, JSON.stringify(list));
      ...
      <div className="price">
        <Form onSubmit={handleSubmit}>
          <InputGroup className="mb-3">
            <InputGroup.Text className="price-Left">Price JOD</InputGroup.Text>
            <Form.Control
              aria-label="Amount (to the nearest dollar)"
              name="Price"
              type="number"
              placeholder="Enter price in JD."
              className="price-mid" required
...
<ul>
          {list.map((item ,idx)=>{
           ...
           return (
             ...
             <h3>Total Price Offered : <span> {item.totalPrice} JOD </span> </h3>
             <h3>Client : {item.client} </h3>
             <h3 id="h33">Client Email : {item.email}</h3>
             <h3>Bid Amount : {item.BidAmount}</h3>
             {auth.user.id===post.userId ?
             
             <button className="btn-Delete" onClick={()=>{deleteBid(item.id)}}>delete</button>
              :<></>}
